...
official, unmodified upstream binaries with no rebuild from source, so one
...
config and layout, which takes a few seconds.
...
to) are intentionally **not** published, because nothing outside the container
needs them directly.
